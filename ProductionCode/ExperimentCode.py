...
def set_trials_sqr(n_reps, direction_vec_sqr,InnerdirVec, coherence_level,DotSpeed,DotSize,FieldSizeSqr,FieldSizeCirc,DotDensity,shuff=True):

    sqrExtras = [45,135,225,315]
    
    opacitycirc=[0,1]
    opacitysqr = [0,1]
    FieldSizeSqr1 = [FieldSizeSqr[0]]
    combinations_with_fixed_opacity = list(itertools.product(direction_vec_sqr, InnerdirVec,[1], [1], coherence_level, DotSize, 
    ...
    combinations_with_variable_opacity_new = list(itertools.product([direction_vec_sqr[0]], InnerdirVec, [0], [1],
                                                                coherence_level, DotSize, DotSpeed,FieldSizeSqr1,FieldSizeCirc,DotDensity,)) 
    
   # combinations_with_variable_opacity_90 = list(itertools.product([direction_vec_sqr[2]], [InnerdirVec[0]], [1], [0],
#                                                                coherence_level, DotSize, DotSpeed,FieldSizeSqr1,FieldSizeCirc,DotDensity,)) 
    
    combinations_forSqrOnly = list(itertools.product(sqrExtras, [InnerdirVec[0]], [1], [0], coherence_level, DotSize, 
                                                             DotSpeed,FieldSizeSqr1,FieldSizeCirc,DotDensity,))
    
    
    combinations_forZeroCoherence_sqr = list(itertools.product([180,0], [0], [1], [0], [0], DotSize, 
                                                             DotSpeed,FieldSizeSqr1,FieldSizeCirc,DotDensity,))
    combinations_forZeroCoherence_circ = list(itertools.product([0], [180,0], [0], [1], [0], DotSize, 
                                                             DotSpeed,FieldSizeSqr1,FieldSizeCirc,DotDensity,))
...
#    
    ...
    all_combinations = combinations_with_variable_opacity_new+modified_list+combinations_forSqrOnly+combinations_forZeroCoherence_sqr+combinations_forZeroCoherence_circ
...
def callAccParameter(win, num_reps_ex
        ...
        sweep_params_block_circ_zeroCoherence = { 'Dir': ([180,0], DIR_IND)
                        ,'opacity': ([1],OPACITY_IND)
                        ,'FieldCoherence': ([0], COHERENCE_IND)
        ...
        rdkCircl_zeroCoherence = init_dot_stim(win
                    ,num_reps_ex
                    ...
                    ,field_shape='sqr'
                    ,stim_name='rdkSqr'
                    ,sweep_params_exp_sqr=sweep_params_block_circ_zeroCoherence
                    ,background_color=color_background
                    ,dot_color=color_dots
                    )
        ...
        sweep_params_block_circ_zeroCoherence_zeroDir = { 'Dir': ([0], DIR_IND)
                        ,'opacity': ([0,0],OPACITY_IND)
                        ,'FieldCoherence': ([0], COHERENCE_IND)
                        ,'dotSize': (dotsize_vec,DOT_SIZE_IND)
                        ,'speed':(dotspeed_vec,SPEED_IND)
                        ,'fieldSize':(fieldSizeCircle_default,FIELD_SIZE_IND)
        ...
        rdkCircl_zeroCoherence_zeroDir = init_dot_stim(win
                    ,num_reps_ex
                    ,field_size=fieldSize_Square
                    ,n_dots= 1
                    ,coher= 1
                    ,field_shape='sqr'
                    ,stim_name='rdkSqr'
                    ,sweep_params_exp_sqr=sweep_params_block_circ_zeroCoherence_zeroDir
                    ,background_color=color_background
                    ,dot_color=color_dots
                    )
        ...
        circle_sweepTable_ZeroCoherence = rdkCircl_zeroCoherence.sweep_table
        circle_sweepTable_ZeroCoherence_zeroDir = rdkCircl_zeroCoherence_zeroDir.sweep_table
        combined_sweepTable_circle = circle_sweepTable + circle_sweepTable_ZeroCoherence+circle_sweepTable_ZeroCoherence_zeroDir
        ...
        FieldCoherence = sweep_params_sqr['FieldCoherence'][0]
        
        sweep_params_block_sqr_specialCase = { 'Dir': ([45,135,225,315], DIR_IND)
                        ,'opacity': ([1],OPACITY_IND)
                        ,'FieldCoherence': (FieldCoherence, COHERENCE_IND)
                        ,'dotSize': (dotsize_vec,DOT_SIZE_IND)
                        ,'speed':(dotspeed_vec,SPEED_IND)
        ...
        rdkSqrSpecialCase = init_dot_stim(win
                    ,num_reps_ex
                    ,field_size=fieldSize_Square
                    ,n_dots= 1
                    ,coher= 1
                    ,field_shape='sqr'
                    ,stim_name='rdkSqr'
                    ,sweep_params_exp_sqr=sweep_params_block_sqr_specialCase
                    ,background_color=color_background
                    ,dot_color=color_dots
                    )
        ...
        sweep_params_block_sqr_zeroCoherence = { 'Dir': ([180,0], DIR_IND)
                        ,'opacity': ([1],OPACITY_IND)
                        ,'FieldCoherence': ([0], COHERENCE_IND)
                        ,'dotSize': (dotsize_vec,DOT_SIZE_IND)
                        ,'speed':(dotspeed_vec,SPEED_IND)
        ...
        rdkSqrZeroCoherence = init_dot_stim(win
                    ,num_reps_ex
                    ,field_size=fieldSize_Square
                    ,n_dots= 1
                    ,coher= 1
                    ,field_shape='sqr'
                    ,stim_name='rdkSqr'
                    ,sweep_params_exp_sqr=sweep_params_block_sqr_zeroCoherence
                    ,background_color=color_background
                    ,dot_color=color_dots
                    )
    
    
        sweep_params_block_sqr_zeroCoherence_zeroDir = { 'Dir': ([0], DIR_IND)
                        ,'opacity': ([0,0],OPACITY_IND)
                        ,'FieldCoherence': ([0], COHERENCE_IND)
                        ,'dotSize': (dotsize_vec,DOT_SIZE_IND)
                        ,'speed':(dotspeed_vec,SPEED_IND)
        ...
        rdkSqrZeroCoherence_zeroDir = init_dot_stim(win
                    ,num_reps_ex
                    ,field_size=fieldSize_Square
                    ,n_dots= 1
                    ,coher= 1
                    ,field_shape='sqr'
                    ,stim_name='rdkSqr'
                    ,sweep_params_exp_sqr=sweep_params_block_sqr_zeroCoherence_zeroDir
                    ,background_color=color_background
                    ,dot_color=color_dots
                    )
        ...
        sqr_sweepTable_specialCase = rdkSqrSpecialCase.sweep_table  
        sqr_sweepTable_ZeroCoherence = rdkSqrZeroCoherence.sweep_table  
        sqr_sweepTable_ZeroCoherence_zeroDir = rdkSqrZeroCoherence_zeroDir.sweep_table  

        combined_sweepTable_sqr = sqr_sweepTable + sqr_sweepTable_specialCase+sqr_sweepTable_ZeroCoherence+sqr_sweepTable_ZeroCoherence_zeroDir
        
        alltrial= set_trials_sqr(
            n_reps=num_reps_ex
            ,direction_vec_sqr=sweep_params_sqr['Dir'][0]
        ...
        sweepOrderCirc,sweepOrderSqr = set_new_trial_orders(
            alltrial
            ,combined_sweepTable_circle
            ,combined_sweepTable_sqr
            )
      ...
      #  list_opacity = np.array([(float(indiv_sweep[OPACITY_IND])) for indiv_sweep in rdkCircle.sweep_table])
        ...
        list_diameters = np.array([[(float(indiv_sweep[FIELD_SIZE_IND])),(float(indiv_sweep[FIELD_SIZE_IND]))] for indiv_sweep in combined_sweepTable_circle])
        list_opacity = np.array([(float(indiv_sweep[OPACITY_IND])) for indiv_sweep in combined_sweepTable_circle])
        indices = np.array(rdkCircle.sweep_order)
        if np.any(indices >= len(list_diameters)):
            raise ValueError("rdkCircle.sweep_order contains an out-of-bound index.")
...
if __name__ == "__main__":
    ...
    fieldSizeCircle_default = [246] # 246 suppose to be 25 deg 
    ...
    opacity_vec = [0,1]
    dirVecCirc = [0,45,90,135,180,225,270,315]
    dirVecSqr =[0,90,180,270]
    coherence_vec = [1]
    dotsize_vec = [40] 
    dotspeed_vec = [10]
